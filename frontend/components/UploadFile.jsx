...
export default function UploadFile() {
      ...
      if (data) {
        ...
        const formattedData = {
          ...
          successful:
            ...
            formattedResults.filter((r) => r.status === "success").length,
          failed:
            data.failed_files ||
            formattedResults.filter((r) => r.status === "error").length,
          duplicates: formattedResults.filter((r) => r.existing_candidate_id)
            .length,
          results: formattedResults,
        };

        setBatchResult(formattedData);
        toast.success(
          `Batch processing completed! ${formattedData.successful} successful, ${formattedData.failed} failed, ${formattedData.duplicates} duplicates`
        );
      }
    },
    onError: (error) => {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case "success":
        return (
          <CheckCircleIcon className="w-5 h-5 text-github-success-fg dark:text-github-success-fg-dark" />
        );
      case "error":
        return (
          <ExclamationCircleIcon className="w-5 h-5 text-github-danger-fg dark:text-github-danger-fg-dark" />
        );
      case "duplicate":
        return (
          <ArrowPathIcon className="w-5 h-5 text-github-attention-fg dark:text-github-attention-fg-dark" />
        );
      default:
        return null;
    }
        ...
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-github-fg-default dark:text-github-fg-default-dark">
            {isDragActive
              ...
              : `Drag and drop ${
                  uploadMode === "batch" ? "resumes" : "resume"
                }, or click to browse`}
          </p>
        ...
        <div className="github-card">
          ...
          <div className="space-y-2 max-h-40 overflow-y-auto">
            {files.map((file) => (
              <div
                key={file.id}
                className="flex items-center justify-between p-2 bg-github-canvas-subtle dark:bg-github-canvas-subtle-dark rounded"
              >
                <div className="flex items-center space-x-2">
                  <DocumentIcon className="w-4 h-4 text-github-accent-fg dark:text-github-accent-fg-dark" />
                  <span className="text-sm text-github-fg-default dark:text-github-fg-default-dark">
                    {file.name}
                  </span>
      ...
      <div className="flex space-x-4">
        ...
        >
          ...
          {isUploading ? (
            ...
            `Upload ${
              uploadMode === "batch" && files.length > 1 ? "Batch" : ""
            } (${files.length} file${files.length > 1 ? "s" : ""})`
          )}
        </button>

        {batchResult && (
          <button onClick={clearAll} className="btn-github">
            Clear Results
          </button>
        )}
      </div>
          ...
          >
            ...
            <div className="github-card bg-github-canvas-subtle dark:bg-github-canvas-subtle-dark">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                <div>
                  <div className="text-2xl font-bold text-github-fg-default dark:text-github-fg-default-dark">
                    {batchResult.total_files}
                  </div>
                  <div className="text-sm text-github-fg-muted dark:text-github-fg-muted-dark">
                    Total
                  </div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-github-success-fg dark:text-github-success-fg-dark">
                ...
                <div>
                  <div className="text-2xl font-bold text-github-danger-fg dark:text-github-danger-fg-dark">
                    {batchResult.failed}
                  </div>
                  <div className="text-sm text-github-danger-fg dark:text-github-danger-fg-dark">
                    Failed
                  </div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-github-attention-fg dark:text-github-attention-fg-dark">
            ...
            <div className="github-card">
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {batchResult.results.map((result, index) => (
                  <div
                    key={index}
                    className="flex items-start justify-between p-3 bg-github-canvas-subtle dark:bg-github-canvas-subtle-dark rounded"
                  >
                    <div className="flex items-start space-x-3">
                      {getStatusIcon(result.status)}
                      <div>
                        <div className="font-medium text-github-fg-default dark:text-github-fg-default-dark">
                          {result.filename}
                        </div>
                        <div className="text-sm text-github-fg-muted dark:text-github-fg-muted-dark">
                          {result.message}
